...
:: -------------------------------
echo Dropping database 'uniq_db' if it exists...
"%XAMPP_PATH%\mysql\bin\mysql.exe" -u root -e "DROP DATABASE IF EXISTS uniq_db;"
:: -------------------------------
:: Datenbank erstellen
:: -------------------------------
echo Erstelle Datenbank 'uniq_db'...
"%XAMPP_PATH%\mysql\bin\mysql.exe" -u root -e "CREATE DATABASE IF NOT EXISTS uniq_db;"

:: -------------------------------
:: Initial-SQL importieren
:: -------------------------------
echo Importiere Initial SQL ...
"%XAMPP_PATH%\mysql\bin\mysql.exe" -u root uniq_db < "%SQL_PATH%db_schema.sql"

:: -------------------------------
:: Restliche Dummy-Daten importieren
:: -------------------------------
echo Importiere Dummy Daten ...
"%XAMPP_PATH%\mysql\bin\mysql.exe" -u root uniq_db < "%SQL_PATH%dummy_data.sql"
